...
class _AgentScreenState extends State<AgentScreen> {
  ...
  void initState() {
    super.initState();
    _fetchAgentDetails();
    // Initialize notifications (AgentScreen is one of the specified screens)
    NotificationScreen.initNotifications(context);
  }

  @override
  void dispose() {
    // Stop the notification timer but do not clear notifications
  ...
  Future<void> _fetchAgentDetails() async {
    ...
    await ExceptionHandling.handleApiCall<Map<String, dynamic>>(
      ...
      onSuccess: (data) async {
        ...
        // Subscribe to FCM topics for the agent
        try {
  ...
  void _logout() async {
    try {
      ...
      // Do not clear notifications on logout; they should persist
      NotificationScreen.dispose();
      if (mounted) {
