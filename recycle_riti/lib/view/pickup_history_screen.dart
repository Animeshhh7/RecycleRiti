// lib/view/pickup_history_screen.dart
...
class _PickupHistoryScreenState extends State<PickupHistoryScreen> {
  ...
  Future<void> _fetchPastRequests() async {
    ...
    try {
      Map<String, dynamic> response = await AuthService.getPickupRequests();
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          Row(
                                            ...
                                            children: [
                                              ...
                                              Container(
                                                ...
                                                decoration: BoxDecoration(
                                                  color: (request['status']?.toString() ?? '').toLowerCase() == 'completed'
                                                      ? Colors.green.withOpacity(0.1)
                                                      : Colors.redAccent.withOpacity(0.1),
                                                  borderRadius: BorderRadius.circular(20),
                                                ),
                                                child: Text(
                                                  request['status']?.toString().capitalize() ?? 'N/A',
                                                  style: TextStyle(
                                                    fontSize: 14,
                                                    color: (request['status']?.toString() ?? '').toLowerCase() == 'completed'
                                                        ? Colors.green
                                          ...
                                          if (_userRole == 'user') ...[
                                            DetailRow(
                                              label: 'Agent',
                                              value: request['agent']?['username']?.toString() ?? 'Not Assigned',
                                            ),
                                            const SizedBox(height: 8),
                                            DetailRow(
                                              label: 'Agent Email',
                                              value: request['agent']?['email']?.toString() ?? 'N/A',
                                            ),
                                            const SizedBox(height: 8),
                                            DetailRow(
                                              label: 'Agent Phone',
                                          ...
                                          DetailRow(
                                            ...
                                            value: request['pickupDate'] != null
                                                ? DateFormat('dd MMM yyyy, HH:mm')
                                                    .format(DateTime.parse(request['pickupDate']))
                                                : 'N/A',
                                          ),
                                          const SizedBox(height: 12),
