...
class _MyRequestsTabState extends State<MyRequestsTab> {
  ...
  void _filterRequests() {
    ...
    if (_searchQuery.isNotEmpty) {
      filtered = filtered.where((request) {
        final agentName = request['assignments'] != null && request['assignments']['agent'] != null
            ? request['assignments']['agent']['username']?.toString().toLowerCase()
            : null;
      ...
      () async {
        ...
        for (var request in selectedRequests) {
          ...
          if (request['status']?.toLowerCase() == 'accepted' || request['status']?.toLowerCase() == 'pending') {
            await AuthService.cancelPickupRequest(requestId);
          } else {
            throw Exception('Only accepted or pending requests can be cancelled');
          }
        }
      },
      defaultErrorMessage: 'Failed to cancel selected requests. Please ensure the request is pending or accepted.',
      onSuccess: () {
        ExceptionHandling.showSnackBar(context, 'Selected requests cancelled successfully');
        _fetchRequests();
      },
      onError: (error) {
        print('CancelSelectedRequests - Error: $error');
                              ...
                              children: [
                                DetailRow(
                                  ...
                                  value: _truncateString(
                                    request['status']?.toString().capitalize() ?? 'Unknown',
                                    15,
                                  ),
                                ...
                                DetailRow(
                                  label: 'Type',
                                  value: _truncateString(
                                    request['recyclableType']?['name']?.toString() ?? 'N/A',
                                    15,
                                  ),
                                  fontSize: 12,
                                ),
                                ...
                                DetailRow(
                                  label: 'Quantity',
                                  value: _truncateString(
                                    '${request['quantity']?.toString() ?? 'N/A'} kg',
                                    15,
                                  ),
                                  fontSize: 12,
                                ),
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                DetailRow(
                                  label: 'Date',
                                  value: _truncateString(formattedDate, 20),
                                  fontSize: 12,
                                ),
                                if (isAccepted || isCompleted || isCancelled) ...[
                                  ...
                                  DetailRow(
                                    label: 'Agent',
                                    value: _truncateString(
                                      request['assignments'] != null && request['assignments']['agent'] != null
                                          ? request['assignments']['agent']['username']?.toString() ?? 'N/A'
                                          : 'No Agent Assigned',
                                      15,
                                    ),
                                    fontSize: 12,
                                  ...
                                  DetailRow(
                                    ...
                                    value: _truncateString(
                                      request['assignments'] != null && request['assignments']['agent'] != null
                                          ? request['assignments']['agent']['email']?.toString() ?? 'N/A'
                                          : 'N/A',
                                      15,
                                    ),
                                    fontSize: 12,
                                  ...
                                  DetailRow(
                                    ...
                                    value: _truncateString(
                                      request['assignments'] != null && request['assignments']['agent'] != null
                                          ? request['assignments']['agent']['phone']?.toString() ?? 'N/A'
                                          : 'N/A',
                                      15,
                                    ),
                                    fontSize: 12,
