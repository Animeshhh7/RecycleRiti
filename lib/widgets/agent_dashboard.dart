...
class _AgentDashboardState extends State<AgentDashboard> {
  ...
  void initState() {
    ...
    // Add notification listener to refresh data if a new pickup request is received
    NotificationScreen.setNotificationCallback(() {
      _fetchPickupRequests();
      print('AgentDashboard: Notification received, refreshing pickup requests');
    });
  }
  ...
  void dispose() {
    ...
    // Remove the notification callback to prevent memory leaks
    NotificationScreen.setNotificationCallback(null);
    super.dispose();
  }
  ...
  Future<void> _fetchPickupRequests() async {
    ...
    await ExceptionHandling.handleApiCall<Map<String, dynamic>>(
      context,
      () => AuthService.getAgentPickupRequests(),
      ...
      onSuccess: (response) {
        if (mounted) {
          setState(() {
            ...
            print('AgentDashboard - Fetched Pickup Requests: ${_pickupRequests.length} requests');
            for (var request in _pickupRequests) {
              print('Request ID: ${request['id']}, Status: ${request['status']}, Assignments: ${request['assignments']}');
            }
  ...
  void _filterRequests() {
    ...
    if (mounted) {
      print('AgentDashboard - Filtered Requests: ${tempRequests.length} requests');
      for (var request in tempRequests) {
        print('Filtered Request ID: ${request['id']}, Status: ${request['status']}');
      }
      setState(() {
  ...
  Future<void> _acceptRequest(String requestId) async {
    ...
    await ExceptionHandling.handleVoidApiCall(
      ...
      onSuccess: () {
        print('AgentDashboard - Successfully accepted request ID: $requestId');
        ExceptionHandling.showSnackBar(context, 'Request accepted successfully');
        setState(() {
          final requestIndex = _pickupRequests.indexWhere((req) => req['id']?.toString() == requestId);
          if (requestIndex != -1) {
            _pickupRequests[requestIndex]['status'] = 'accepted';
            _pickupRequests[requestIndex]['assignments'] = {
              'agent': {'id': widget.currentAgentId}
            };
            print('AgentDashboard - Updated local state for request ID: $requestId, Status: accepted, Assignments: ${_pickupRequests[requestIndex]['assignments']}');
            _filterRequests();
          }
        });
        _fetchPickupRequests();
  ...
  Future<void> _cancelRequest(String requestId) async {
    ...
    if (request['assignments'] != null) {
      assignedAgentId = request['assignments']['agent']?['id']?.toString();
    }
    final bool isAssignedToAgent = assignedAgentId != null && assignedAgentId == widget.currentAgentId;
    ...
    await ExceptionHandling.handleVoidApiCall(
      ...
      onSuccess: () async {
        ...
        setState(() {
          final requestIndex = _pickupRequests.indexWhere((req) => req['id']?.toString() == requestId);
          if (requestIndex != -1) {
            _pickupRequests[requestIndex]['status'] = 'cancelled';
            _pickupRequests[requestIndex]['assignments'] = null;
            print('AgentDashboard - Updated local state for request ID: $requestId, Status: cancelled, Assignments: ${_pickupRequests[requestIndex]['assignments']}');
            _filterRequests();
          }
        });
        _fetchPickupRequests();
      ...
      onError: (error) {
        ...
        } else if (error.contains('400') || error.contains('Cannot cancel')) {
          ExceptionHandling.showSnackBar(context, 'Cannot cancel this request. It may have already been completed or cancelled.');
        } else {
          ExceptionHandling.showSnackBar(context, error);
        }
      },
  ...
  Future<void> _deleteRequest(String requestId) async {
    ...
    final isDeleted = await NotificationManager.isNotificationDeleted(requestId);
    print('AgentDashboard - Notification $requestId deletion verified: $isDeleted');
  ...
  Future<void> _markAsCompleted(String requestId) async {
    ...
    String? requestStatus = request['status']?.toString().toLowerCase();
    if (requestStatus != 'accepted') {
      ExceptionHandling.showSnackBar(context, 'Only accepted requests can be marked as completed');
      return;
    }
    ...
    final bool isAssignedToAgent = assignedAgentId != null && assignedAgentId == widget.currentAgentId;
    if (!isAssignedToAgent) {
      ExceptionHandling.showSnackBar(context, 'You are not authorized to complete this request');
      return;
    }
    ...
    await ExceptionHandling.handleVoidApiCall(
      ...
      onError: (error) {
        if (error.contains('403')) {
          ExceptionHandling.showSnackBar(context, 'Not authorized to complete this request. Please contact support if this issue persists.');
        } else if (error.contains('400') || error.contains('Only accepted requests')) {
          ExceptionHandling.showSnackBar(context, 'Only accepted requests can be marked as completed.');
        } else {
          ExceptionHandling.showSnackBar(context, error);
        }
  ...
  Future<void> _makePhoneCall(String? phoneNumber) async {
    ...
    if (await canLaunchUrl(phoneUri)) {
      ...
      print('AgentDashboard - Initiated phone call to $phoneNumber');
    } else {
      ExceptionHandling.showSnackBar(context, "Unable to make a phone call");
      print('AgentDashboard - Failed to initiate phone call to $phoneNumber');
    }
  }
  ...
  Future<void> _sendMessage(String? phoneNumber) async {
    ...
    if (await canLaunchUrl(smsUri)) {
      ...
      print('AgentDashboard - Initiated SMS to $phoneNumber');
    } else {
      ExceptionHandling.showSnackBar(context, "Unable to send a message");
      print('AgentDashboard - Failed to initiate SMS to $phoneNumber');
    }
  }
  ...
  void _viewRequestDetails(String requestId) {
    ...
    try {
      ...
      print('AgentDashboard - Navigated to track-request-agent with request ID: $requestId');
    } catch (e) {
      print('AgentDashboard - Navigation error to track-request-agent with request ID: $requestId, Error: $e');
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            _errorMessage ?? 'An error occurred while fetching requests',
  ...
  Widget _buildRequestTile(Map<String, dynamic> request, int index) {
    ...
    final bool isPending = requestStatus == 'pending' && request['assignments'] == null;
    final bool isAccepted = requestStatus == 'accepted' && isAssignedToAgent;
    final bool isCompleted = requestStatus == 'completed';
    final bool isCancelled = requestStatus == 'cancelled';

    print('AgentDashboard - Rendering tile for Request ID: ${request['id']}, Status: $requestStatus, Assigned Agent ID: $assignedAgentId, Current Agent ID: ${widget.currentAgentId}, isAssignedToAgent: $isAssignedToAgent, isAccepted: $isAccepted');
    ...
    if (request['location'] != null) {
      try {
        ...
        print('AgentDashboard - Location parsing error for Request ID: ${request['id']}: $e');
      }
    } else {
      locationError = 'Location not available';
      print('AgentDashboard - Location not available for Request ID: ${request['id']}');
    }
                                  ...
                                  child: flutter_map.FlutterMap(
                                    ...
                                    options: flutter_map.MapOptions(
                                      ...
                                      onMapReady: () {
                                        ...
                                        print('AgentDashboard - Map displayed for Request ID: ${request['id']}, Location: lat:$userLat, lng:$userLng');
                                      },
                                    ),
                                    children: [
                        ...
                        child: locationError != null
                            ...
                            : flutter_map.FlutterMap(
                                options: flutter_map.MapOptions(
                                  ...
                                  initialZoom: 15,
                                  onMapReady: () {
                                    print('AgentDashboard - Mini map displayed for Request ID: ${request['id']}, Location: lat:$userLat, lng:$userLng');
                                  },
                                ),
                                children: [
                                  flutter_map.TileLayer(
                ...
                Row(
                  ...
                  children: [
                    ...
                    if (isPending) ...[
                      CustomButton(
                        onPressed: () {
                          print('AgentDashboard - Accept button pressed for request ID: ${request['id']}');
                          _acceptRequest(request['id']?.toString() ?? '');
                        },
                        color: Colors.orange,
                        isMini: true,
                        child: const Text(
                    ...
                    if (isAccepted) ...[
                      CustomButton(
                        onPressed: () {
                          print('AgentDashboard - Cancel button pressed for request ID: ${request['id']}');
                          _cancelRequest(request['id']?.toString() ?? '');
                        },
                        color: Colors.redAccent,
                        isMini: true,
                        child: const Text(
                      ...
                      CustomButton(
                        onPressed: () {
                          print('AgentDashboard - Complete button pressed for request ID: ${request['id']}');
                          _markAsCompleted(request['id']?.toString() ?? '');
                        },
                        color: AppTheme.primaryColor,
                        isMini: true,
                        child: const Text(
                    ...
                    if (isCompleted || isCancelled) ...[
                      CustomButton(
                        onPressed: () {
                          print('AgentDashboard - Delete button pressed for request ID: ${request['id']}');
                      ...
                      CustomButton(
                        onPressed: () {
                          print('AgentDashboard - View button pressed for request ID: ${request['id']}');
                          _viewRequestDetails(request['id']?.toString() ?? '');
                        },
                        color: AppTheme.primaryColor,
                        isMini: true,
