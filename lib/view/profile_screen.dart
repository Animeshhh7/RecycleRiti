    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      NotificationScreen.initNotifications(
        ...
        onNotificationReceived: () {
          setState(() {});
          print('ProfileScreen: Notification received, refreshing UI');
        },
      );
    });
  }
    ...
    await ExceptionHandling.handleApiCall<void>(
      ...
      () async {
        ...
        if (token == null || token.isEmpty) {
          print('ProfileScreen: No access token found, redirecting to login');
      ...
      onSuccess: (_) {
        print('ProfileScreen: Session verified, user details loaded');
      },
      onError: (error) {
        if (mounted) {
          setState(() {
      ...
      onSuccess: (data) {
        if (mounted) {
          setState(() {
            ...
            print('ProfileScreen: Successfully fetched user profile - Username: $_userName, Email: $_userEmail, Role: $_userRole');
          });
        }
      },
      onError: (error) {
        if (mounted) {
          ...
          print('ProfileScreen: Error fetching user profile: $error');
        }
      },
    );
    ...
    if (confirm == true) {
      await ExceptionHandling.handleVoidApiCall(
        ...
        onSuccess: () {
          ...
          print('ProfileScreen: User logged out successfully');
        },
        onError: (error) {
          print('ProfileScreen: Logout failed: $error');
        },
      );
    }
  }
    ...
    return Scaffold(
      appBar: CustomAppBar(
        ...
        onNotificationStateChanged: () {
          setState(() {});
          print('ProfileScreen: Notification state changed, refreshing UI');
        },
        onBackPressed: () {
          Provider.of<BottomNavController>(context, listen: false).changeIndex(0);
          print('ProfileScreen: Back button pressed, navigated to HomeScreen');
        },
      ),
      body: Container(
